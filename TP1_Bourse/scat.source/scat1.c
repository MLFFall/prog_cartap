...
//    "                                        ",
	...
	"l'alimentation de la carte a été coupée, communication ultérieure impossible avec elle",
	"la carte ne repond pas au reset",
	"la carte est inaccessible pour cause d'autre connexion en cours",
	"absence de carte dans le lecteur",
...
int endianness;
/*
unsigned int endian_swap32(unsigned int var)
{
	var = (var >> 24) +
		((var & 0x00ff0000) >> 8) +
		((var & 0x0000ff00) << 8) +
...
/*i Combinaisons possibles de l'état rendu par SCardStatus
...
*/
	i=pool_max-=l+1;
	if (i<=pool_ptr) fatal_error("mémoire pleine");
...
//récuprationt d'une ligne par la console
...
{
	FILE*f;
	if (istack_ptr>=INPUT_STACK_SIZE) error("débordement de la pile d'entrée");
	f=input_stack[istack_ptr].file=fopen(name,"r");
	if (f==NULL)
	{
		sprintf(msgdata,"erreur à l'ouverture du fichier %s",name);
		error(msgdata);
	}
...
void check_stack()
{
	if (stack_ptr==100) error("pile pleine");
}
...
//		break;
	case '*':
		...
		sr=llmul(mem+mem_ptr, sa, mem+a, sb, mem+b);
		for (i=0;i<sr;i++)
		{
			mem[a+i]=mem[mem_ptr+i];
		}
		stack_ptr--;
		stack[stack_ptr-1].len=sr;
		break;
...
// test si le caractère c est dans la liste des caractères de fin ends
// si ends == NULL, seul le caractère \0 est admissislbe
// rend 0 si c n'est pas un caractère de fin
// rend 1 si c est un caractère de fin
int isinends(char c, char*ends)
{
//printf("[%c(%d) %s]",c,c,ends);
	if (ends==NULL) return c==0;
	while (*ends) if (c==*ends++) return 1;
	return 0;	
}
...
void push_expr(char* ends)
{
	skip_blanks();
	push_terml();
	while (isinends(last_char=skip_blanks(),ends)==0)
	{
		push_terml();
		// concaténer
...
//printf("ope %s  input %c%c\n",ope, last_char,input_string[tk_ptr]);
	if (last_char!=ope[0]) return 0;
	if (ope[1]==0)
	{
//		tk_ptr++;
		return 1;
	}
	if (input_string[tk_ptr]==ope[1])
	{
		tk_ptr++;
		return 1;
	}
...
// évalue une expression booléenne et rend sa valeur
int bool_fact()
{
	int ope;
	int sa, sb;
	...
	// analyse syntaxique de l'expression et affecte ope selon l'opérateur
	push_expr("=><!");
	if (is_ope("=="))
	{
		ope=0;
	...
	// il faut tester >= et <= avant > et < en raison de l'analyse
	else if (is_ope(">="))
	{
		ope=2;
	}
	...
	{
		unexpected_char(input_string[tk_ptr]);
	}
	push_expr(")");
//	check_char(')');
...
void macro()
{
	...
	//int t;	// index dans la table de hachage
